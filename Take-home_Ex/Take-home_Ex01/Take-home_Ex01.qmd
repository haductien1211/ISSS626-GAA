...
Now, we can re-run density() using the resale data set and plot the output kde map. In this case I'll be using bw.diggle() since it seems to be working best
...
Both graph produce roughly the same results which highlight some of the hot zone of accidents that could happen throughout the period
