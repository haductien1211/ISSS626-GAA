...
Before we can prepare the choropleth map, we need to combine both the geospatial data object (i.e. `shan_sf`) and aspatial data.frame object (i.e. ict_derived) into one. This will be performed by using the [left_join](https://dplyr.tidyverse.org/reference/mutate-joins.html) function of `dplyr` package. The `shan_sf` simple feature data.frame will be used as the base data object and the `ict_derived` *data.frame* will be used as the join table.
...
The code chunks below are used to create the data visualisation. They consist of two main parts. First to create the choropleth map then use `tmap_arrange()` to group these map together.

```{r, fig.height=9, fig.width=12, dpi=100}
tmap_mode('plot')
RADIO_PR <- qtm(shan_sf, "RADIO_PR")
LLPHONE_PR <- qtm(shan_sf, "LLPHONE_PR")
TV_PR <- qtm(shan_sf, "TV_PR")
MPHONE_PR <- qtm(shan_sf, "MPHONE_PR")
COMPUTER_PR <- qtm(shan_sf, "COMPUTER_PR")
INTERNET_PR <- qtm(shan_sf, "INTERNET_PR")

tmap_arrange(RADIO_PR, LLPHONE_PR, TV_PR, MPHONE_PR, COMPUTER_PR, INTERNET_PR)
```
...
**Some portion of the map goes into 127.6 to 211.0 range and has a different color compared to the previously plotted `qtm(shan_sf, "RADIO_PR")`**
...
```{r, fig.height=5, fig.width=12, dpi=100}
...
**Min-Max standardisation kept the value within the 0-1 range while Z-score standardisation could go to negative and concentrated around 0**
...
The [gap](https://hastie.su.domains/Papers/gap.pdf) statistic compares the total within intra-cluster variation for different values of k with their expected values under null reference distribution of the data. The estimate of the optimal clusters will be value that maximize the gap statistic (i.e., that yields the largest gap statistic). This means that the clustering structure is far away from the random uniform distribution of points.
...
Next, we can visualise the plot by using [fviz_gap_stat()](https://rpkgs.datanovia.com/factoextra/reference/fviz_nbclust.html) of `factoextra` package.
...
In the code chunk below, the [heatmaply()](https://talgalili.github.io/heatmaply/reference/heatmaply.html) of [heatmaply](https://cran.r-project.org/web/packages/heatmaply/index.html) package is used to build an interactive cluster heatmap.
...
The output is called *group*s. It is a list object.
...
Note that the dissimilarity matrix must be an object of class dist, i.e. an object obtained with the function `dist()`. For sample code chunk, please refer to [Hands-on Exercise 2.2 Computing proximity matrix](https://isss626-gaa-2024aug-tien.netlify.app/hands-on_ex/hands-on_ex02/hands-on_ex02.2)
...
```{r, fig.height=5, fig.width=18, dpi=100}
