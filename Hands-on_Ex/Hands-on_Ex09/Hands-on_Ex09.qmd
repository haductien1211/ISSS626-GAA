...
# 1. Introduction & Learning Outcome
...
-   `ELDERCARE`: GIS data showing location of eldercare service.
    This [data](https://data.gov.sg/collections/714/view) is downloaded from data.gov.sg.
...
-   `OD_Matrix`: a distance matrix in csv format.
    ...
    -   `estination_id`: the unique id values of the destination (i.e. `fid` of `ELDERCARE` data set.),
...
**Reminder**: Except `MP14_SUBZONE_NO_SEA_PL` data set, the other three data set are specially prepared by Prof. Kam for teaching and research purpose.
Students taking IS415 Geospatial Analytics and Applications are allowed to use them for hands-on exercise purpose.
Please obtain formal approval from Prof. Kam if you want to use them for other courses or usage.
