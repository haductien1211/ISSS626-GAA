...
Notice that only the first ten records will be displayed as default
...
`tmap` provides a total ten data classification methods, namely: f*ixed, sd, equal, pretty (default), quantile, kmeans, hclust, bclust, fisher,* and *jenks*.
...
Different classification methods supported by tmap changes the **DEPENDENCY** range of data
:::
...
Different classes numbers supported by tmap changes the **DEPENDENCY** number of data ranges
:::
...
tm_shape(mpsz_pop2020)+
  tm_fill("DEPENDENCY", 
          ...
          palette = "Blues",
          title = "No. of persons") +
  tm_layout(main.title = "Distribution of Dependency 
  Ratio by planning subzone",
            main.title.position = "center",
            main.title.size = 1.2,
            legend.height = 0.45, 
            legend.width = 0.35,
  ...
  Authorithy (URA) and Population data from Department of Statistics DOS", 
             position = c("left", "bottom"))
```
...
```{r, fig.width = 15, fig.height = 16, dpi = 200}
tm_shape(mpsz_pop2020)+
...
-   [tmap: changes](https://cran.r-project.org/web/packages/tmap/vignettes/tmap-changes.html)
